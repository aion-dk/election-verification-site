...
export function useConferenceConnector(slug: string) {
  ...
  conferenceApi.value.interceptors.response.use(responseHandler, responseErrorHandler);

  return {
    conferenceClient: {
      async getStatus() {
        return (await conferenceApi.value.get(`/${parsedSlug}/status`)) as ElectionStatusResponse;
      },
      async getStylingData() {
        return (await conferenceApi.value.get(`/${parsedSlug}/theme`)) as string;
      },
      async getTranslationsData(locale: string) {
        ...
        return currentTranslationsData.value?.translations[locale].js.ballot_verification_site;
      },
    },
  };
}
